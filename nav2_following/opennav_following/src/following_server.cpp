...
nav2_util::CallbackReturn FollowingServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  following_action_server_ = std::make_unique<FollowingActionServer>(
    node, "follow_object", std::bind(&FollowingServer::followObject, this), nullptr, std::chrono::milliseconds(500),
    true);
  ...
  filtered_dynamic_pose_pub_ =
    create_publisher<geometry_msgs::msg::PoseStamped>("filtered_dynamic_pose", rclcpp::SensorDataQoS());
...
nav2_util::CallbackReturn FollowingServer::on_activate(const rclcpp_lifecycle::State & /*state*/)
{
...
nav2_util::CallbackReturn FollowingServer::on_deactivate(const rclcpp_lifecycle::State & /*state*/)
{
...
nav2_util::CallbackReturn FollowingServer::on_cleanup(const rclcpp_lifecycle::State & /*state*/)
{
...
nav2_util::CallbackReturn FollowingServer::on_shutdown(const rclcpp_lifecycle::State &)
{
...
void FollowingServer::getPreemptedGoalIfRequested(
  ...
  const typename std::shared_ptr<nav2_util::SimpleActionServer<ActionT>> & action_server)
{
...
bool FollowingServer::checkAndWarnIfCancelled(
  typename std::shared_ptr<nav2_util::SimpleActionServer<ActionT>> & action_server, const std::string & name)
{
  ...
  try {
    ...
    if (target_frame.empty()) {
      if (pose_topic.empty()) {
        ...
        dynamic_pose_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>(
          ...
          });  // Only want the most recent pose
        lock.lock();
      }
    } else {
    ...
    while (rclcpp::ok()) {
      try {
        ...
        RCLCPP_WARN(get_logger(), "Following failed, will retry");
  ...
  geometry_msgs::msg::PoseStamped & object_pose, const std::string & target_frame)
{
  ...
  for (const double & target_angle : angles) {
    ...
    while (rclcpp::ok()) {
      ...
      command->twist = computeRotateToHeadingCommand(
...
geometry_msgs::msg::Twist FollowingServer::computeRotateToHeadingCommand(
  const double & angular_distance_to_heading, const geometry_msgs::msg::Twist & current_velocity, const double & dt)
{
  geometry_msgs::msg::Twist cmd_vel;
  const double sign = angular_distance_to_heading > 0.0 ? 1.0 : -1.0;
  const double rotate_to_heading_angular_vel_ = 1.0;
  const double rotate_to_heading_max_angular_accel_ = 1.57;
